...
【P1-3 操作建议风险分级规范】
操作建议必须按风险等级分类输出，明确标注执行顺序：

**优先级1：不停机远程核查**（可立即执行，无安全风险）
- 定值核对：通过EMS或本地监控核对保护参数（如频率保护下限F_low_set寄存器值）
- 趋势分析：查看历史曲线，确认是否为频繁性越限
- 配置参数：检查采样系数、变比配置、量程设置
- 日志分析：查看SOE事件记录、故障录波数据

**优先级2：需停机电测**（需停机或现场操作，有一定风险）
- 采样回路：检查PT/CT接线端子松动、信号线屏蔽层接地状态
...
- 未确认频繁性越限前，禁止建议具体的定值修改（如"调整至49.5Hz"）
- 调整定值前必须检查是否违反GB/T 19964标准，标注"需电网公司审批"
